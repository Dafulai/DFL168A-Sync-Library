...
bool J1939Class::getVIN(String &VIN)
{ 
  char bufferx[20];
  char HexValue[]="0x00";
  byte i;
  
  if (!Pareant->DigitalCommand("FEEC")) {  return false;}
...
bool J1939Class::getDTC(byte &DTC_Num, long  (&SPN)[5], byte (&FMI)[5], byte (&CM)[5],byte (&OC)[5], byte DTCFormat=1 )
{
  String StrDTC;
  int len,i;
  ...
  if (!Pareant->DigitalCommand("FECA")) {  return false;}
  Pareant->HandleResponse(Pareant->ReturnStr); 
  ...
  for (i=4;i<=len-8;i=i+8)
  {
     ...
     if (0xffffffff==DTC4bytes) return true;
     ...
     DTC_Num++;
     if (DTC_Num>=5) return true; //in case over the array max index
...
boolean J1939Class::clearDTC()
{
  if (!Pareant->DigitalCommand("FED3")) {  return false;}
  return true;
}

boolean PGN65262Class::refresh()
{
  if (!Pareant->DigitalCommand("FEEE")) { SuccessFresh=false; return false;}  
  Pareant->HandleResponse(Pareant->ReturnStr);  
...
boolean PGN65267Class::refresh()
{
  if (!Pareant->DigitalCommand("FEF3")) { SuccessFresh=false; return false;}  
  Pareant->HandleResponse(Pareant->ReturnStr);  
  (Pareant->ReturnStr).trim();
  FreshResultStr=(Pareant->ReturnStr);
  SuccessFresh=true;
...
boolean PGN65256Class::refresh()
{
  if (!Pareant->DigitalCommand("FEE8")) { SuccessFresh=false; return false;}    
  Pareant->HandleResponse(Pareant->ReturnStr);  
  (Pareant->ReturnStr).trim();
  FreshResultStr=(Pareant->ReturnStr);
  SuccessFresh=true;
...
boolean PGN65269Class::refresh()
{
  if (!Pareant->DigitalCommand("FEF5")) { SuccessFresh=false; return false;}    
  Pareant->HandleResponse(Pareant->ReturnStr);  
  (Pareant->ReturnStr).trim();
  FreshResultStr=(Pareant->ReturnStr);
  SuccessFresh=true;
...
boolean PGN65257Class::refresh()
{
  if (!Pareant->DigitalCommand("FEE9")) { SuccessFresh=false; return false;}  
  Pareant->HandleResponse(Pareant->ReturnStr);  
  (Pareant->ReturnStr).trim();
  FreshResultStr=(Pareant->ReturnStr);
  SuccessFresh=true;
...
boolean PGN61444Class::refresh()
{
  if (!Pareant->DigitalCommand("F004")) { SuccessFresh=false; return false;}  
  Pareant->HandleResponse(Pareant->ReturnStr);  
  (Pareant->ReturnStr).trim();
  FreshResultStr=(Pareant->ReturnStr);
  SuccessFresh=true;
...
//bool PGN57344Class::getSeatBelt(bool buckled)   //Mar 07 2019 fix bug
...
//version 1.0.2 add -----start
bool PGN64996Class::refresh()
{
	if (!Pareant->DigitalCommand("FDE4")) { SuccessFresh = false; return false; }
	Pareant->HandleResponse(Pareant->ReturnStr);
	(Pareant->ReturnStr).trim();
	FreshResultStr = (Pareant->ReturnStr);
	SuccessFresh = true;
...
bool PGN61445Class::refresh()
{
	if (!Pareant->DigitalCommand("F005")) { SuccessFresh = false; return false; }
	Pareant->HandleResponse(Pareant->ReturnStr);
	(Pareant->ReturnStr).trim();
	FreshResultStr = (Pareant->ReturnStr);
	SuccessFresh = true;
